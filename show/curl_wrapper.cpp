...
struct Init
{
   ...
   Init() noexcept
   {
	  if(CURLcode const cc = curl_global_init(CURL_GLOBAL_ALL); cc!= 0)
	  {
...
auto percent_encode(std::string_view const unescaped) -> std::string
{
   ...
   std::unique_ptr<char, void (*)(void*)> curl_string(curl_easy_escape(curl_ctx.get(), unescaped.data(), unescaped.length()), curl_free);
   if(!curl_string)
   {
	  std::exit(EXIT_FAILURE);
   }
