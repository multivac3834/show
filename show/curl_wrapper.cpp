...
auto percent_encode(std::string_view const unescaped) -> std::string
{
	...
	if(!curl_ctx) { std::exit(EXIT_FAILURE); }

	std::unique_ptr<char, void (*)(void*)> curl_string(
		curl_easy_escape(curl_ctx.get(), unescaped.data(), static_cast<int>(unescaped.length())), curl_free);
	if(!curl_string) { std::exit(EXIT_FAILURE); }

	std::string escaped = curl_string.get();
	return escaped;
}
...
auto http_get(std::string const& url) -> Http_response
{
	...
	struct
	{
		...
		void operator=(CURLcode ec)
		{
			if(ec != CURLE_OK)
			{
				std::cout << (buffer.data() != nullptr ? std::string(buffer.data())
													   : "Curl error, but error buffer is empty");
...
auto Http_response::get_status_code() const noexcept -> int
{
	...
	if(pos_sc == std::string_view::npos) { return -1; }
